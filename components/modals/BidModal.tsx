...
import { X, CheckCircle, Minus, Plus, Paperclip, FileText, Loader2, Save, Bold, Italic, List as ListIcon, ListOrdered, Lock } from 'lucide-react';
...
const RichTextEditor = ({ value, onChange, placeholder, height = "h-32" }: { value: string, onChange: (val: string) => void, placeholder?: string, height?: string }) => {
    ...
    return (
        <div className={`flex flex-col border rounded-sm bg-surface transition-colors overflow-hidden ${isFocused ? 'border-accent' : 'border-ink/20'}`}>
            ...
            <div
                ref={editorRef}
                className={`w-full p-4 overflow-auto outline-none ${height} font-sans text-sm text-ink empty:before:content-[attr(data-placeholder)] empty:before:text-ink/30 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5`}
                contentEditable
                onInput={handleInput}
...
interface BidModalProps {
    ...
    initialData?: BidData; // Support pre-filling for updates
}
...
export const BidModal: React.FC<BidModalProps> = ({ isOpen, onClose, questTitle, maxReward, currency, exchangeRate = 1, onBidSubmit, initialData }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (initialData) {
                ...
                setDuration('');
                setProposal('');
                setContact('researcher@institute.edu'); // Default pre-filled
                setIsPublic(false);
                setFiles([]);
            }
    ...
    const handleFormSubmit = () => {
        ...
        // Direct submission - no password verification needed (wallet already connected)
        setStep('SUCCESS');
                                ...
                                <div>
                                    ...
                                    <div className="relative flex items-center">
                                        <button 
                                            ...
                                            className="absolute left-0 top-0 bottom-0 w-12 flex items-center justify-center text-ink/40 hover:text-ink hover:bg-stone/5 transition-colors border-r border-ink/20 z-10"
                                        >
                                            <Minus size={16} />
                                        </button>
                                        <input 
                                            ...
                                            className="w-full border border-ink/20 py-4 px-12 font-mono text-lg font-bold text-ink focus:outline-none focus:border-accent rounded-sm text-center"
                                        />
                                        <button 
                                            type="button"
                                            onClick={() => {
                                            ...
                                            className="absolute right-0 top-0 bottom-0 w-12 flex items-center justify-center text-ink/40 hover:text-ink hover:bg-stone/5 transition-colors border-l border-ink/20 z-10"
                                        >
                                            <Plus size={16} />
                                        </button>
                                    </div>
